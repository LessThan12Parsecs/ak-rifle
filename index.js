...
export default e => {
  ...
  const {Left_wrist, Right_wrist} = modelBones;
  ...
  const shootingSoundIndex = soundFiles.combat.map(sound => sound.name).indexOf('combat/Colt45_Shot2.wav');
  const dryClipSoundIndex = soundFiles.combat.map(sound => sound.name).indexOf('combat/gun_dry_fire.wav');
  const reloadSoundIndex = soundFiles.combat.map(sound => sound.name).indexOf('combat/ak47_reload.wav');
  ...
  let clipSize = 30;
  let ammoInClip = clipSize; // Standard Size of ammo clip is 30
  let totalAmmo = 90; // 3 clips
  let ammoNeeded = 0;
  ...
  function reloadAmmo(){
    ...
    if (ammoInClip < clipSize && totalAmmo > 0) {
      ammoNeeded = clipSize - ammoInClip;
      localPlayer.addAction(newAction);
      sounds.playSound(soundFiles.combat[reloadSoundIndex]);
  ...
  e.waitUntil((async () => {
    ...
    {
      let u2 = `${baseUrl}ak-withhandle.glb`;
      if (/^https?:/.test(u2)) {
        u2 = '/@proxy/' + u2;
      }
      const m = await metaversefile.import(u2);
      ...
      const handguardBone = gunApp.getObjectByName('handleguard');
      handguardBone.originalPosition = handguardBone.position.clone();
      gunApp.handguardBone = handguardBone;
    ...
    if (!wearing) {
      ...
      if (gunApp) {
        ...
        if(isShooting && ((timestamp - lastShootTime) > fireDelay)) {
          ...
          if (ammoInClip > 0) {
            sounds.playSound(soundFiles.combat[shootingSoundIndex]);
          }
          else {
            sounds.playSound(soundFiles.combat[dryClipSoundIndex]);
          }
          gunApp.use();
        }
      }
